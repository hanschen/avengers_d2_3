...
The satellite retrievals were averaged to superobservations (superobs) at the same resolution as the atmospheric transport model (about 25 km).
...
\begin{figure}[tb]
    ...
    \caption{Number of synthetic XCO2 observations from a hypothetical CO2M satellite in
        ...
        to superobs at the atmospheric transport model grid (about 25 km resolution).
    }
    \label{fig:xco2}
\end{figure}

"The XCO2 observation values were derived from the truth simulation by calculating the mass-weighted column-averaged CO2 dry-air mole fraction.
...
For real cases,
one would likely also apply thinning creating superobs for larger areas to reduce the impact of spatially correlated systematic errors
...
Synthetic XCO2 observations were then sampled from this truth simulations and perturbed with random noise of 1 ppm for each 25 km $\times$ 25 km superobs.
